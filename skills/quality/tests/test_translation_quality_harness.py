...
def test_harness_rejects_japanese_punctuation_in_korean_prose(tmp_path: Path) -> None:
    ...
    locale_failures = [
        issue
        for issue in report["hard_failures"]
        if issue["message"] == "Japanese punctuation found in Korean prose."
    ]
    assert report["status"] == "reject"
    assert {issue["reason"].split("`")[1] for issue in locale_failures} == {"。", "、"}
    assert all("code。、" not in issue["target_span"] for issue in locale_failures)
...
def test_harness_allows_japanese_punctuation_inside_protected_code(tmp_path: Path) -> None:
    ...
    assert not any(
        issue["message"] == "Japanese punctuation found in Korean prose."
